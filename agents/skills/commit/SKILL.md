...
This skill is part of the development workflow pipeline: `issue` → `implement` → `test` → `commit` → `pr` → `review`. This skill is the **fourth** stage. The implement, test, and commit steps are iterative — they can be invoked multiple times for a given issue.
...
The user MUST be prompted with the next pipeline step: "Ready to create the PR? Run the `pr` skill with the issue number to review the changes and open a draft pull request." DO NOT proceed on your own.
