...
Measured on a fresh paths-mode round: the assembled prompt went from 138,587 to 85,790 bytes, roughly 34.6k to 21.4k tokens — a 38% reduction. `tests/test_prompt_budget.py` budgets the assembled prompt, the combined skill-plus-rationale size, and every other skill.
