...
Each pass also captures the code it reviewed, into `snapshot-<#>/` beside the document. Findings cite `file:line` in a repo whose history gets rewritten before merge, so the commits a review ran against don't survive — the snapshot anchors on the merge-base with the upstream default branch, which does, and carries everything above it as a patch plus a content-addressed tree SHA. Restore it into any clone that can reach the base and the tree SHA proves you got back exactly what was reviewed.

Each finding-set mutation lands as its own commit whose message says why — `review: Close B2 — nil guard now present at server.py:318` — so the document's git history is the record of how the review evolved. Those commits go to the repository you declare as `review-repo` in `.sdlc/config.json`, resolved relative to that file just like `guides-dir`. Leave it unset and `.sdlc` is used when it is already a repository; when it isn't, the agent asks you whether to name one or run `git init .sdlc`, then records your answer so it only asks once. It never goes hunting up the filesystem for a repository you didn't name. Pass `target` (or set `review-branch` in `.sdlc/config.json`) to land the commits on a specific branch; the agent commits through a temporary worktree so the tree under review is never touched, while still writing the document where the rest of the pipeline reads it. While blocking findings remain, address them with `sdlc_implement <target> --review <#>` and run another pass. When you're satisfied, mark the PR ready for review and merge via GitHub.
